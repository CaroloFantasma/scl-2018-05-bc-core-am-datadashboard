...
window.sortUsers = (users, orderBy, orderDirection) => { 
  if (orderBy === 'name') { 
    return users.sort(function(ab, cd) {
      if (orderDirection === 'ASC') {
        return ab.name.localeCompare(cd.name);
      } else {
        return ab.name.localeCompare(cd.name) * -1;
      }
    });
  }

  if (orderBy === 'percent') {
    return users.sort((ab, cd)=> {
      if (orderDirection === 'ASC') {
        return ab.stats.percent - cd.stats.percent;
      } else {
        return (ab.stats.percent - cd.stats.percent) * -1;
      }
    });
  }
};
